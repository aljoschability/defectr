	...
	public static void main(String[] args) {
		...
		scale.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				TaskItem item = getTaskBarItem();
				if (item != null)
					item.setProgress(scale.getSelection());
			}
		});
		...
		button.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				TaskItem item = getTaskBarItem();
				if (item != null)
					item.setOverlayText(text.getText());
			}
		});
		button = new Button(group, SWT.PUSH);
		...
		Listener listener3 = new Listener() {
			public void handleEvent(Event event) {
				...
				if (item != null) {
					...
					Image image = null;
					if (!text.equals("NONE")) {
						// InputStream is = Snippet336.class.getResourceAsStream(text);
						// System.out.println(is);
						// image = new Image(display, is);
						image = ImageDescriptor.createFromFile(getClass(), text).createImage(display);
					}
		...
		button.setSelection(true);
		String[] images = { "icons/control.png", "pause.gif", "run.gif", "warning.gif" };
